...
function generateGuide(taskId, ctx) {
  const from = ctx?.from || "旧住所";
  const to = ctx?.to || "新住所";
  const date = ctx?.date || "引越し日";
  const situation = ctx?.profile?.situation;
  const hasCar = ctx?.profile?.car === "car";
  const hasFamily = situation === "family";
  const isCouple = situation === "couple";

  const guides = {
    "w5": `【いつ】引越し前14日以内 or 引越し当日\n【どこで】${from}の市区町村窓口（または マイナポータルでオンライン手続き可）\n【持ち物】本人確認書類・印鑑・マイナンバーカード（あれば）${hasFamily ? "・家族全員分の書類" : ""}\n【注意】転出証明書は転入届に必要なので必ず受け取ること。マイナポータルなら24時間手続き可能。`,

    "a1": `【いつ】${date}から14日以内（期限厳守）\n【どこで】${to}の市区町村窓口\n【持ち物】転出証明書・本人確認書類・印鑑・マイナンバーカード（あれば）${hasFamily ? "・家族全員分の書類" : ""}\n【注意】転入届を出すと同時にマイナンバー住所変更・健康保険・年金も一緒に手続きできる。まとめて済ませよう。`,

    "a2": `【いつ】転入届と同時が効率的\n【どこで】${to}の市区町村窓口\n【持ち物】マイナンバーカード・本人確認書類\n【注意】マイナンバーカードの住所変更は引越し後90日以内。放置するとカードが失効するので注意。`,

    "a3": `【いつ】転入届と同時に\n【どこで】${to}の市区町村窓口\n【持ち物】旧住所の国民健康保険証・本人確認書類${hasFamily ? "・家族全員分の保険証" : ""}\n【注意】会社員の場合は勤務先の人事へ。自営・フリーランスは必ず役所で手続きを。`,

    "a4": `【いつ】転入届と同時が効率的\n【どこで】${to}の市区町村窓口\n【持ち物】マイナンバーカード・年金手帳（あれば）\n【注意】マイナンバー連携済みなら自動更新の場合も。念のため窓口で確認を。`,

    "a5": `【いつ】なるべく早く（法律上は速やかに）\n【どこで】${to}近くの警察署または運転免許センター\n【持ち物】免許証・新住所の住民票（転入届後に取得）・本人確認書類\n【注意】住所変更しないまま運転すると道路交通法違反になる場合がある。転入届の翌日以降に動くとスムーズ。`,

    "a6": `【いつ】住所変更後15日以内（法律上の義務）\n【どこで】${to}管轄の陸運局\n【持ち物】車検証・新住所の住民票・印鑑${hasCar ? "・車庫証明（新住所の保管場所が変わる場合）" : ""}\n【注意】車庫証明は警察署で先に取得が必要な場合も。1週間ほどかかるので早めに動く。`,

    "b8": `【いつ】引越し日の3日前までに予約\n【どこで】新居管轄のガス会社に電話またはWeb\n【持ち物】開栓当日は本人立ち会いが必要\n【注意】開栓は必ず立ち会いが必要。引越し当日か翌日に予約しておくと安心。予約が遅れると数日間ガスが使えない。`,

    "b11": `【いつ】今すぐ（引越し1〜2ヶ月前が理想）\n【どこで】現在の回線会社に電話またはWeb\n【持ち物】契約書・回線番号\n【注意】光回線の工事は混んでいると1〜2ヶ月待ちになることも。${to}への引越しが決まったら最優先で動くこと。`,

    "b16": `【いつ】引越し1週間前まで\n【どこで】郵便局窓口 or e転居（https://www.post.japanpost.jp）でオンライン手続き可\n【持ち物】本人確認書類・印鑑（窓口の場合）\n【注意】e転居なら24時間オンラインで手続き可能。${from}から${to}への転送を1年間設定しておくと安心。`,
  };

  return guides[taskId] || `【${from}→${to}への引越しの場合】\nこの手続きは引越し後なるべく早く済ませましょう。不明な点は${to}の市区町村窓口に確認してください。`;
}
      ...
      {open && (
        <div className="ti-detail">
          ...
          <div className="ti-action-row">
            ...
            <button className="ti-ask-btn" onClick={()=>onAsk(task)} >
              {task.personalGuide ? "✦ あなたの場合を確認" : "✦ ガイドを見る"}
            </button>
          </div>
          {task.aiTip && (
...
function ChatPanel({ ctx, onClose }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      const lastMsg = apiMsgs[apiMsgs.length-1]?.content?.toLowerCase() || "";
      let reply = "";
      if (lastMsg.includes("転入") || lastMsg.includes("てんにゅう")) {
        reply = `転入届について：\n\n【いつ】引越し後14日以内\n【どこで】${ctx.to || "新住所"}の市区町村窓口\n【持ち物】転出証明書・本人確認書類・印鑑・マイナンバーカード\n\nマイナポータル（myna.go.jp）からオンラインで転出届を出すと、来庁予約もできて便利です。`;
      } else if (lastMsg.includes("転出") || lastMsg.includes("てんしゅつ")) {
        reply = `転出届について：\n\n【いつ】引越し前14日以内または当日\n【どこで】${ctx.from || "旧住所"}の市区町村窓口、またはマイナポータルでオンライン手続き可\n【持ち物】本人確認書類・印鑑\n\n転出証明書は転入届に必要なので必ず受け取ってください。`;
      } else if (lastMsg.includes("ガス") || lastMsg.includes("gas")) {
        reply = `ガスの開栓について：\n\n開栓には必ず本人の立ち会いが必要です。引越し日の3日前までにガス会社へ予約を。予約が遅れると数日間ガスが使えません。\n\n新居のガス会社は管理会社または物件情報で確認できます。`;
      } else if (lastMsg.includes("マイナンバー") || lastMsg.includes("mynumber")) {
        reply = `マイナンバーカードの住所変更：\n\n転入届と同時に窓口で手続きできます。引越し後90日以内に手続きしないとカードが失効するので注意。\n\n【持ち物】マイナンバーカード・本人確認書類`;
      } else if (lastMsg.includes("免許") || lastMsg.includes("めんきょ")) {
        reply = `運転免許証の住所変更：\n\n【どこで】新住所近くの警察署または運転免許センター\n【持ち物】免許証・新住所の住民票・本人確認書類\n\n住民票は転入届後に取得できます。住所変更しないまま運転すると違反になる場合があります。`;
      } else if (lastMsg.includes("郵便") || lastMsg.includes("転送")) {
        reply = `郵便の転送手続き：\n\ne転居（https://www.post.japanpost.jp）からオンラインで無料手続きできます。${ctx.from || "旧住所"}から${ctx.to || "新住所"}への転送を1年間設定できます。引越し1週間前までに手続きを。`;
      } else {
        reply = `${ctx.from || "旧住所"}から${ctx.to || "新住所"}への引越しですね。\n\n何の手続きについて知りたいですか？\n\n・転出届・転入届\n・ガス・電気・水道\n・マイナンバー\n・運転免許証\n・郵便転送\n\n知りたい手続きを教えてください！`;
      }
...
export default function App() {
  ...
  const askAI = (task) => {
    const tip = generateGuide(task.id, ctx);
    const next = {};
          ...
          return (
            <div key={cat} style={{marginBottom:24}}>
              ...
              <div style={{border:"1px solid rgba(28,43,58,0.13)",borderRadius:2,overflow:"hidden",background:"white",boxShadow:"0 1px 8px rgba(28,43,58,0.08)"}}>
                {items.map(task => <TaskItem key={task.id} task={task} onToggle={toggle} onAsk={askAI} loading={null} ctx={ctx}/>)}
              </div>
            </div>
          );
        });
